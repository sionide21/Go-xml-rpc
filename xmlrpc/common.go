...
func (f Fault) String() string { return fmt.Sprintf("%s (%d)", f.FaultString, f.FaultCode) }
...
func (s StructValue) LoadXML(parser *xml.Parser) (ParamValue, os.Error) {
	...
	for {
		...
		for x := 0; x < 2; x++ {
			...
			if !ok {
				return nil, error(fmt.Sprintf("Unexpected Token: %v, (%+T)", t, t, x))
			}
...
func (f Fault) LoadXML(parser *xml.Parser) (ParamValue, os.Error) {
	...
	if !ok || start.Name.Local != "value" {
		return nil, error("Unexpected symbol")
	}
	...
	if !(ok1 && ok2 && ok3) {
		return nil, error("Invalid fault response")
	}
...
func parseMessage(p *xml.Parser) (ParamValue, os.Error) {
	...
	start, ok := t.(xml.StartElement)
	if !ok {
		return nil, error(fmt.Sprintf("Unexpected symbol: %v", t))
	}
	...
	default:
		return nil, error(fmt.Sprintf("Unknown type: %s", start.Name.Local))
	}
	return nil, nil
}
